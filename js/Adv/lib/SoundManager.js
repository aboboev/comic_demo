...
SoundManager.prototype.load = function (key, cb) {
    this.key = key;
...
SoundManager.prototype.resourceLoad = function (Idx) {
    ...
    if (src[this._path[Idx]] === undefined) {
        src = {};
        src[decodeURI(this._path[Idx])] = resources[decodeURI(this._path[Idx])];
    }
...
SoundManager.prototype.Play = function (alias, loop, volume) {
    ...
    for (var i = 0; i < this._path.length; i++) {
        ...
        if (this._path[i].indexOf(decodeURI(alias)) !== -1)
            index = i;
