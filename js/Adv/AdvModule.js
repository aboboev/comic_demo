...
var createMokomokoFukidashi = function (args) {
    ...
    if(addDeg === undefined)
    {
        var sum = 0;
        // var rand = [];
        var rand = [0.06745221697751033, 0.067312173030342, 0.06478834325234338, 0.08649923005034629, 0.07388758983239978, 0.09053916966389154, 0.05825297011792787, 0.08661066514295734, 0.05834446986064359, 0.050097296054158844, 0.05002879600246479, 0.07771792145652734, 0.07082253041035864, 0.08874325829483964, 0.06405475687248484];
        var i = 0;
        while (true) {
    ...
    for (var i = 0; i < addDeg.length - 1; i++) {
        deg += addDeg[i];
        if(args.randoms === undefined)
        {
            //random = getRandomArbitrary(20, 30);      
...
var mokoDraw = function (size, x, y, type, deg) {
    ...
    if (type === "moko") {
        ...
        */
        ...
        randoms = [157, 153, 169, 147, 162, 151, 167, 177, 151, 177, 154, 148, 154, 166, 166, 142, 161, 162]
        ...
        for (var i = 0; i < ctx.graphicsData[0].shape.points.length; i += 2) {
            points.push(new PIXI.Point(ctx.graphicsData[0].shape.points[i], ctx.graphicsData[0].shape.points[i + 1]));
        }
